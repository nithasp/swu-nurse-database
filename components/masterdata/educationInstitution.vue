<template>
   <div class="p-1.5">
      <div class="flex justify-between items-center mb-5 flex-wrap gap-y-3">
         <div class="bg-white rounded-xl px-4 py-2 flex items-center gap-x-2">
            ...
            <h1 class="text-xl">สถาบันการศึกษา</h1>
         </div>
      ...
      <div class="bg-white rounded-lg p-4 mt-4">
         ...
         <UTable
            ...
            :columns="[
               ...
               { key: 'nameTh', label: 'รายการสถาบันการศึกษา', class: 'min-w-[200px]', sortable: true },
   ...
   <DialogEducationInstitutionForm
      :isOpen="isOpen"
      :setDialog="setDialog"
...
import DialogEducationInstitutionForm from "~/components/dialog/masterdata/educationInstitutionForm";
import DialogConfirm from "~/components/dialog/confirm";
import Pagination from "~/components/pagination";
import MenuDropdown from "~/components/form/menu";
import Loading from "~/components/loading";

import useMasterEducationInstitutionAPI from "~/service/api/masterData/educationInstitution";
const { fetchMasterEducationInstitutionList, deleteEducationInstitution } = useMasterEducationInstitutionAPI;
...
const getListTable = async () => {
   const res = await fetchMasterEducationInstitutionList(filters.value);
   if (res.code == 20000) {
      tableData.value = [];
      for (let i = 0; i < res.responseObject.educationalInstitutionList.length; i++) {
         const data = res.responseObject.educationalInstitutionList[i];
...
const fnDelete = async () => {
   ...
   const res = await deleteEducationInstitution(masterId.value);
   if (res.code == 20000) {
