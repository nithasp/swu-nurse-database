   ...
   <div class="p-1.5">
      <div class="flex justify-between items-center mb-5 flex-wrap gap-y-3">
         <div class="bg-white rounded-xl px-4 py-2 flex items-center gap-x-2">
            ...
            <h1 class="text-xl">ตำแหน่งงาน</h1>
         </div>
      ...
      <div class="bg-white rounded-lg p-4 mt-4">
         ...
         <UTable
            ...
            :columns="[
               ...
               { key: 'nameTh', label: 'ตำแหน่งงาน', class: 'min-w-[200px]', sortable: true },
...
import DialogPositionForm from "~/components/dialog/masterdata/positionForm";
import DialogConfirm from "~/components/dialog/confirm";
import Pagination from "~/components/pagination";
import MenuDropdown from "~/components/form/menu";
import Loading from "~/components/loading";

import useMasterPositionAPI from "~/service/api/masterData/position";
const { fetchMasterPositionList, deletePosition } = useMasterPositionAPI;
...
const getListTable = async () => {
   const res = await fetchMasterPositionList(filters.value);
   if (res.code == 20000) {
      tableData.value = [];
      for (let i = 0; i < res.responseObject.positionList.length; i++) {
         const data = res.responseObject.positionList[i];
...
const fnDelete = async () => {
   ...
   const res = await deletePosition(masterId.value);
   if (res.code == 20000) {
