   ...
   <div class="p-1.5">
      <div class="flex justify-between items-center mb-5 flex-wrap gap-y-3">
         <div class="bg-white rounded-xl px-4 py-2 flex items-center gap-x-2">
            ...
            <h1 class="text-xl">ระดับฐานข้อมูลวารสาร</h1>
         </div>
      ...
      <div class="bg-white rounded-lg p-4 mt-4">
         ...
         <UTable
            ...
            :columns="[
               ...
               { key: 'nameTh', label: 'รายการระดับฐานข้อมูลวารสาร', class: 'min-w-[200px]', sortable: true },
...
import DialogImpactFactorForm from "~/components/dialog/masterdata/impactFactorForm";
import DialogConfirm from "~/components/dialog/confirm";
import Pagination from "~/components/pagination";
import MenuDropdown from "~/components/form/menu";
import Loading from "~/components/loading";

import useMasterImpactFactorAPI from "~/service/api/masterData/impactFactor";
const { fetchMasterImpactFactorList, deleteImpactFactor } = useMasterImpactFactorAPI;
...
const getListTable = async () => {
   const res = await fetchMasterImpactFactorList(filters.value);
   if (res.code == 20000) {
      tableData.value = [];
      for (let i = 0; i < res.responseObject.impactFactorList.length; i++) {
         const data = res.responseObject.impactFactorList[i];
...
const fnDelete = async () => {
   ...
   const res = await deleteImpactFactor(masterId.value);
   if (res.code == 20000) {
