   ...
   <div class="p-1.5">
      <div class="flex justify-between items-center mb-5 flex-wrap gap-y-3">
         <div class="bg-white rounded-xl px-4 py-2 flex items-center gap-x-2">
            ...
            <h1 class="text-xl">คำนำหน้าชื่อ</h1>
         </div>
      ...
      <div class="bg-white rounded-lg p-4 mt-4">
         ...
         <UTable
            ...
            :columns="[
               ...
               { key: 'nameTh', label: 'คำนำหน้าชื่อ (ไทย)', class: 'min-w-[200px]', sortable: true },
               { key: 'nameEn', label: 'คำนำหน้าชื่อ (อังกฤษ)', class: 'min-w-[200px]', sortable: true },
...
import DialogPrefixForm from "~/components/dialog/masterdata/prefixForm";
...
const { fetchMasterPrefixList, deletePrefix } = useMasterPrefixAPI;
...
const getListTable = async () => {
   const res = await fetchMasterPrefixList(filters.value);
   if (res.code == 20000) {
      tableData.value = [];
      for (let i = 0; i < res.responseObject.prefix.length; i++) {
         const data = res.responseObject.prefix[i];
...
const fnDelete = async () => {
   ...
   const res = await deletePrefix(masterId.value);
   if (res.code == 20000) {
