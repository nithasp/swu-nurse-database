   ...
   <div class="p-1.5">
      <div class="flex justify-between items-center mb-5 flex-wrap gap-y-3">
         <div class="bg-white rounded-xl px-4 py-2 flex items-center gap-x-2">
            ...
            <h1 class="text-xl">สถานภาพบุคลากร</h1>
         </div>
      ...
      <div class="bg-white rounded-lg p-4 mt-4">
         ...
         <UTable
            ...
            :columns="[
               ...
               { key: 'nameTh', label: 'สถานภาพบุคลากร', class: 'min-w-[200px]', sortable: true },
...
import DialogPersonnelStatusForm from "~/components/dialog/masterdata/personnelStatusForm";
...
const { fetchMasterPersonnelStatusList, deletePersonnelStatus } = useMasterPersonnelStatusAPI;
...
const getListTable = async () => {
   const res = await fetchMasterPersonnelStatusList(filters.value);
   if (res.code == 20000) {
      tableData.value = [];
      for (let i = 0; i < res.responseObject.personnelStatusList.length; i++) {
         const data = res.responseObject.personnelStatusList[i];
...
const fnDelete = async () => {
   ...
   const res = await deletePersonnelStatus(masterId.value);
   if (res.code == 20000) {
