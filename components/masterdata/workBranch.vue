      ...
      <div class="bg-white rounded-lg p-4 mt-4">
         ...
         <UTable
            ...
            :columns="[
               ...
               { key: 'nameTh', label: 'งาน', class: 'min-w-[200px]', sortable: true },
...
import DialogWorkBranchForm from "~/components/dialog/masterdata/workBranchForm";
import DialogConfirm from "~/components/dialog/confirm";
import Pagination from "~/components/pagination";
import MenuDropdown from "~/components/form/menu";
import Loading from "~/components/loading";

import useMasterWorkBranchAPI from "~/service/api/masterData/workBranch";
const { fetchMasterWorkBranchList, deleteWorkBranch } = useMasterWorkBranchAPI;
...
const getListTable = async () => {
   const res = await fetchMasterWorkBranchList(filters.value);
   if (res.code == 20000) {
      tableData.value = [];
      for (let i = 0; i < res.responseObject.workBranchList.length; i++) {
         const data = res.responseObject.workBranchList[i];
...
const fnDelete = async () => {
   ...
   const res = await deleteWorkBranch(masterId.value);
   if (res.code == 20000) {
