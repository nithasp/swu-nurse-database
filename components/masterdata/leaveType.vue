   ...
   <div class="p-1.5">
      <div class="flex justify-between items-center mb-5 flex-wrap gap-y-3">
         <div class="bg-white rounded-xl px-4 py-2 flex items-center gap-x-2">
            ...
            <h1 class="text-xl">ประเภทการลา</h1>
         </div>
      ...
      <div class="bg-white rounded-lg p-4 mt-4">
         ...
         <UTable
            ...
            :columns="[
               ...
               { key: 'parent', label: 'ประเภทการลา', class: 'min-w-[200px]', sortable: true },
               { key: 'nameTh', label: 'รายละเอียดการลา', class: 'min-w-[200px]', sortable: true },
         ...
         >
            ...
            <template #parent-data="{ row }">
               <div class="max-w-[200px] line-clamp-2">
                  {{ row.parent == "educationalLeave" ? "ลาศึกษาต่อ" : row.parent == "annualLeave" ? "ลาประจำปี" : "-" }}
               </div>
            </template>
...
import DialogLeaveTypeForm from "~/components/dialog/masterdata/leaveTypeForm";
...
const { fetchMasterLeaveTypeList, deleteLeaveType } = useMasterLeaveTypeAPI;
...
const getListTable = async () => {
   const res = await fetchMasterLeaveTypeList(filters.value);
   if (res.code == 20000) {
      tableData.value = [];
      for (let i = 0; i < res.responseObject.leaveTypeList.length; i++) {
         const data = res.responseObject.leaveTypeList[i];
...
const fnDelete = async () => {
   ...
   const res = await deleteLeaveType(masterId.value);
   if (res.code == 20000) {
