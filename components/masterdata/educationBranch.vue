      ...
      <div class="bg-white rounded-lg p-4 mt-4">
         ...
         <UTable
            ...
            :columns="[
               ...
               { key: 'nameTh', label: 'รายการสาขาวิชา', class: 'min-w-[200px]', sortable: true },
...
import DialogEducationBranchForm from "~/components/dialog/masterdata/educationBranchForm";
import DialogConfirm from "~/components/dialog/confirm";
import Pagination from "~/components/pagination";
import MenuDropdown from "~/components/form/menu";
import Loading from "~/components/loading";

import useMasterEducationBranchAPI from "~/service/api/masterData/educationBranch";
const { fetchMasterEducationBranchList, deleteEducationBranch } = useMasterEducationBranchAPI;
...
const getListTable = async () => {
   const res = await fetchMasterEducationBranchList(filters.value);
   if (res.code == 20000) {
      tableData.value = [];
      for (let i = 0; i < res.responseObject.educationalBranchList.length; i++) {
         const data = res.responseObject.educationalBranchList[i];
...
const fnDelete = async () => {
   ...
   const res = await deleteEducationBranch(masterId.value);
   if (res.code == 20000) {
