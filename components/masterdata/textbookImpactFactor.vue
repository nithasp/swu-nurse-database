<template>
   <div class="p-1.5">
      <div class="flex justify-between items-center mb-5 flex-wrap gap-y-3">
         <div class="bg-white rounded-xl px-4 py-2 flex items-center gap-x-2">
            ...
            <h1 class="text-xl">ระดับฐานข้อมูลหนังสือ/ตำรา</h1>
         </div>
      ...
      <div class="bg-white rounded-lg p-4 mt-4">
         ...
         <UTable
            ...
            :columns="[
               ...
               { key: 'nameTh', label: 'รายการระดับฐานข้อมูลหนังสือ/ตำรา', class: 'min-w-[200px]', sortable: true },
   ...
   <DialogTextbookImpactFactorForm
      :isOpen="isOpen"
      :setDialog="setDialog"
...
import DialogTextbookImpactFactorForm from "~/components/dialog/masterdata/textbookImpactFactorForm";
import DialogConfirm from "~/components/dialog/confirm";
import Pagination from "~/components/pagination";
import MenuDropdown from "~/components/form/menu";
import Loading from "~/components/loading";

import useMasterTextbookImpactFactorAPI from "~/service/api/masterData/textbookImpactFactor";
const { fetchMasterTextbookImpactFactorList, deleteTextbookImpactFactor } = useMasterTextbookImpactFactorAPI;
...
const getListTable = async () => {
   const res = await fetchMasterTextbookImpactFactorList(filters.value);
   if (res.code == 20000) {
      tableData.value = [];
      for (let i = 0; i < res.responseObject.textBookImpactFactorList.length; i++) {
         const data = res.responseObject.textBookImpactFactorList[i];
...
const fnDelete = async () => {
   ...
   const res = await deleteTextbookImpactFactor(masterId.value);
   if (res.code == 20000) {
