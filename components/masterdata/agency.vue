   ...
   <div class="p-1.5">
      <div class="flex justify-between items-center mb-5 flex-wrap gap-y-3">
         <div class="bg-white rounded-xl px-4 py-2 flex items-center gap-x-2">
            ...
            <h1 class="text-xl">หน่วยงานที่ปฏิบัติงาน</h1>
         </div>
      ...
      <div class="bg-white rounded-lg p-4 mt-4">
         ...
         <UTable
            ...
            :columns="[
               ...
               { key: 'nameTh', label: 'หน่วยงานที่ปฏิบัติงาน', class: 'min-w-[200px]', sortable: true },
               { key: 'amount', label: 'จำนวนตำแหน่งงาน', class: 'min-w-[200px]', sortable: true },
...
import DialogAgencyForm from "~/components/dialog/masterdata/agencyForm";
...
const { fetchMasterAgencyList, deleteAgency } = useMasterAgencyAPI;
...
const getListTable = async () => {
   const res = await fetchMasterAgencyList(filters.value);
   if (res.code == 20000) {
      tableData.value = [];
      for (let i = 0; i < res.responseObject.agencyList.length; i++) {
         const data = res.responseObject.agencyList[i];
...
const fnDelete = async () => {
   ...
   const res = await deleteAgency(masterId.value);
   if (res.code == 20000) {
