   ...
   <div class="p-1.5">
      <div class="flex justify-between items-center mb-5 flex-wrap gap-y-3">
         <div class="bg-white rounded-xl px-4 py-2 flex items-center gap-x-2">
            ...
            <h1 class="text-xl">ประเภทสัญญา</h1>
         </div>
      ...
      <div class="bg-white rounded-lg p-4 mt-4">
         ...
         <UTable
            ...
            :columns="[
               ...
               { key: 'nameTh', label: 'ประเภทสัญญา', class: 'min-w-[200px]', sortable: true },
...
import DialogContractTypeForm from "~/components/dialog/masterdata/contractTypeForm";
import DialogConfirm from "~/components/dialog/confirm";
import Pagination from "~/components/pagination";
import MenuDropdown from "~/components/form/menu";
import Loading from "~/components/loading";

import useMasterContractTypeAPI from "~/service/api/masterData/contractType";
const { fetchMasterContractTypeList, deleteContractType } = useMasterContractTypeAPI;
...
const getListTable = async () => {
   const res = await fetchMasterContractTypeList(filters.value);
   if (res.code == 20000) {
      tableData.value = [];
      for (let i = 0; i < res.responseObject.contractTypeList.length; i++) {
         const data = res.responseObject.contractTypeList[i];
...
const fnDelete = async () => {
   ...
   const res = await deleteContractType(masterId.value);
   if (res.code == 20000) {
