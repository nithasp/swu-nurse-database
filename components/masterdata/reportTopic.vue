   ...
   <div class="p-1.5">
      <div class="flex justify-between items-center mb-5 flex-wrap gap-y-3">
         <div class="bg-white rounded-xl px-4 py-2 flex items-center gap-x-2">
            ...
            <h1 class="text-xl">หัวข้อรายงานการศึกษาต่อ</h1>
         </div>
      ...
      <div class="bg-white rounded-lg p-4 mt-4">
         ...
         <UTable
            ...
            :columns="[
               ...
               { key: 'nameTh', label: 'หัวข้อรายงานการศึกษาต่อ', class: 'min-w-[200px]', sortable: true },
...
import DialogReportTopicForm from "~/components/dialog/masterdata/reportTopicForm";
import DialogConfirm from "~/components/dialog/confirm";
import Pagination from "~/components/pagination";
import MenuDropdown from "~/components/form/menu";
import Loading from "~/components/loading";

import useMasterReportTopicAPI from "~/service/api/masterData/reportTopic";
const { fetchMasterReportTopicList, deleteReportTopic } = useMasterReportTopicAPI;
...
const getListTable = async () => {
   const res = await fetchMasterReportTopicList(filters.value);
   if (res.code == 20000) {
      tableData.value = [];
      for (let i = 0; i < res.responseObject.reportTopicList.length; i++) {
         const data = res.responseObject.reportTopicList[i];
...
const fnDelete = async () => {
   ...
   const res = await deleteReportTopic(masterId.value);
   if (res.code == 20000) {
