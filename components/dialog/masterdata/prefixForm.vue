<template>
   ...
   >
      <div class="p-6">
         <p class="text-primary-500 text-xl">{{ actionMode == "edit" ? "แก้ไข" : actionMode == "create" ? "เพิ่ม" : "" }}คำนำหน้าชื่อ</p>
         ...
         <div class="grid grid-cols-1 sm:grid-cols-none justify-center gap-y-3 sm:gap-2">
            <div class="grid sm:grid-cols-[150px_1fr] gap-x-3 gap-y-1">
               <p class="text-sm sm:text-right mt-2">คำนำหน้าชื่อ(ไทย)<span class="text-red-500"> *</span> :</p>
               <Input
                  placeholder="คำนำหน้าชื่อ(ไทย)"
            ...
            <div class="grid sm:grid-cols-[150px_1fr] gap-x-3 gap-y-1">
               <p class="text-sm sm:text-right mt-2">คำนำหน้าชื่อ(อังกฤษ)<span class="text-red-500"> *</span> :</p>
               <Input
                  placeholder="คำนำหน้าชื่อ(อังกฤษ)"
                  class="flex flex-col sm:w-[260px]"
                  v-model="form.nameEn"
                  heightClass="h-[38px]"
                  :requireInput="{ showMessage: validator.nameEn?.showMessage, message: validator.nameEn?.message }"
                  @change="(event) => clearRequireOnChange({ value: event.target.value, key: 'nameEn' })"
                  :disabled="actionMode == 'view'"
               />
            </div>
...
import useMasterPrefixAPI from "~/service/api/masterData/prefix";
const { fetchPrefix, createPrefix, updatePrefix } = useMasterPrefixAPI;
...
const getDataById = async () => {
   const res = await fetchPrefix(props.id);
   if (res.code == 20000) {
      form.value = res.responseObject.prefix;
   }
};
...
const fnCreate = async () => {
   ...
   const res = await createPrefix(form.value);
   if (res.code == 20000) {
      props.getListTable();
...
const fnUpdate = async () => {
   ...
   const res = await updatePrefix(form.value);
   if (res.code == 20000) {
      props.getListTable();
      const toast = useToast();
      toast.add({ title: res.message, color: "green" });
