<template>
   ...
   >
      <div class="p-6">
         <p class="text-primary-500 text-xl">{{ actionMode == "edit" ? "แก้ไข" : actionMode == "create" ? "เพิ่ม" : "" }}งาน</p>
         ...
         <div class="grid grid-cols-1 sm:grid-cols-none justify-center gap-y-3 sm:gap-2">
            <div class="grid sm:grid-cols-[150px_1fr] gap-x-3 gap-y-1">
               <p class="text-sm sm:text-right mt-2">งาน<span class="text-red-500"> *</span> :</p>
               <Input
                  placeholder="งาน"
...
const { fetchWorkBranch, createWorkBranch, updateWorkBranch } = useMasterWorkBranchAPI;
...
const getDataById = async () => {
   const res = await fetchWorkBranch(props.id);
   if (res.code == 20000) {
      form.value = res.responseObject.workBranch;
   }
};
...
const fnCreate = async () => {
   ...
   const res = await createWorkBranch(form.value);
   if (res.code == 20000) {
      props.getListTable();
...
const fnUpdate = async () => {
   ...
   const res = await updateWorkBranch(form.value);
   if (res.code == 20000) {
      props.getListTable();
      const toast = useToast();
      toast.add({ title: res.message, color: "green" });
