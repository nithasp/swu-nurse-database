<template>
   ...
   >
      <div class="p-6">
         <p class="text-primary-500 text-xl">{{ actionMode == "edit" ? "แก้ไข" : actionMode == "create" ? "เพิ่ม" : "" }}ระดับฐานข้อมูลหนังสือ/ตำรา</p>
         ...
         <div class="grid grid-cols-1 sm:grid-cols-none justify-center gap-y-3 sm:gap-2">
            <div class="grid sm:grid-cols-[180px_1fr] gap-x-3 gap-y-1">
               <p class="text-sm sm:text-right mt-2">ระดับฐานข้อมูลหนังสือ/ตำรา<span class="text-red-500"> *</span> :</p>
               <Input
                  placeholder="ระดับฐานข้อมูลหนังสือ/ตำรา"
...
const { fetchTextbookImpactFactor, createTextbookImpactFactor, updateTextbookImpactFactor } = useMasterTextbookImpactFactorAPI;
...
const getDataById = async () => {
   const res = await fetchTextbookImpactFactor(props.id);
   if (res.code == 20000) {
      form.value = res.responseObject.textBookImpactFactor;
   }
};
...
const fnCreate = async () => {
   ...
   const res = await createTextbookImpactFactor(form.value);
   if (res.code == 20000) {
      props.getListTable();
...
const fnUpdate = async () => {
   ...
   const res = await updateTextbookImpactFactor(form.value);
   if (res.code == 20000) {
      props.getListTable();
      const toast = useToast();
      toast.add({ title: res.message, color: "green" });
