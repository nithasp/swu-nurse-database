<template>
   ...
   >
      <div class="p-6">
         <p class="text-primary-500 text-xl">{{ actionMode == "edit" ? "แก้ไข" : actionMode == "create" ? "เพิ่ม" : "" }}ตำแหน่งงาน</p>
         ...
         <div class="grid grid-cols-1 sm:grid-cols-none justify-center gap-y-3 sm:gap-2">
            <div class="grid sm:grid-cols-[150px_1fr] gap-x-3 gap-y-1">
               <p class="text-sm sm:text-right mt-2">ตำแหน่งงาน<span class="text-red-500"> *</span> :</p>
               <Input
...
const { fetchPosition, createPosition, updatePosition } = useMasterPositionAPI;
...
const getDataById = async () => {
   const res = await fetchPosition(props.id);
   if (res.code == 20000) {
      form.value = res.responseObject.position;
   }
};
...
const fnCreate = async () => {
   ...
   const res = await createPosition(form.value);
   if (res.code == 20000) {
      props.getListTable();
...
const fnUpdate = async () => {
   ...
   const res = await updatePosition(form.value);
   if (res.code == 20000) {
      props.getListTable();
      const toast = useToast();
      toast.add({ title: res.message, color: "green" });
