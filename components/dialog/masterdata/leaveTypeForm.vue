<template>
   ...
   >
      <div class="p-6">
         <p class="text-primary-500 text-xl">{{ actionMode == "edit" ? "แก้ไข" : actionMode == "create" ? "เพิ่ม" : "" }}ประเภทการลา</p>
         ...
         <div class="grid grid-cols-1 sm:grid-cols-none justify-center gap-y-3 sm:gap-2">
            <div class="grid sm:grid-cols-[150px_1fr] gap-x-3 gap-y-1">
               <p class="text-sm sm:text-right mt-2">ประเภทการลา<span class="text-red-500"> *</span> :</p>
               <Select
                  class="w-[260px]"
                  v-model="form.parent"
                  :options="leaveList"
                  ...
                  :searchable="false"
                  :requireInput="{ showMessage: validator.parent?.showMessage, message: validator.parent?.message }"
                  :disabled="actionMode == 'view'"
               />
            </div>
            <div class="grid sm:grid-cols-[150px_1fr] gap-x-3 gap-y-1">
               <p class="text-sm sm:text-right mt-2">รายละเอียดการลา<span class="text-red-500"> *</span> :</p>
               <Input
...
import useMasterLeaveTypeAPI from "~/service/api/masterData/leaveType";
const { fetchLeaveType, createLeaveType, updateLeaveType } = useMasterLeaveTypeAPI;
...
const getDataById = async () => {
   const res = await fetchLeaveType(props.id);
   if (res.code == 20000) {
      form.value = res.responseObject.leaveType;
   }
};
...
watch(
   () => form.value.parent,
   (newData) => {
      clearRequireOnChange({ value: newData, key: "parent" });
   }
);
...
const fnCreate = async () => {
   ...
   const res = await createLeaveType(form.value);
   if (res.code == 20000) {
      props.getListTable();
...
const fnUpdate = async () => {
   ...
   const res = await updateLeaveType(form.value);
   if (res.code == 20000) {
      props.getListTable();
      const toast = useToast();
      toast.add({ title: res.message, color: "green" });
