<template>
   <div class="bg-primary-500 rounded-t-lg h-[38px] px-4 flex items-center justify-between">
      <h2 class="text-white text-xl">ข้อมูลการศึกษา</h2>
   ...
   <div class="bg-white px-[26px] py-8">
      <UTable
         ...
         :columns="[
            ...
            {
               key: 'educationCertification',
               label: 'วุฒิการศึกษา',
               class: 'min-w-[100px]',
            },
            {
               key: 'educationalBranch',
               label: 'สาขาวิชาที่สำเร็จ',
               class: 'min-w-[100px]',
            },
            {
   ...
   <DialogConfirm
      ...
      icon="danger"
      :btnAction="handleDeletePersonnelEducation"
   />
   <DialogEducationalInformationForm
      :isOpen="isDialogEducationalInformationFormOpen"
      :setDialog="setDialogEducationalInformationForm"
      :actionMode="mode"
...
import DialogEducationalInformationForm from "~/components/dialog/personal-work/educationalInformationForm";
...
const { fetchPersonnelEducationList, deletePersonnelEducation } = usePersonnelEducationAPI;
...
const userEducationId = ref(null);
...
const getListTable = async () => {
   ...
   const res = await fetchPersonnelEducationList(props?.personnelId, filters.value);
   if (res?.code === 20000) {
...
const isDialogEducationalInformationFormOpen = ref(false);
const setDialogEducationalInformationForm = (status, type) => {
   isDialogEducationalInformationFormOpen.value = status;
   if (type === "toEditMode") {
      openDialogMode.value = "edit";
      setDialogEducationalInformationForm(true);
   }
   if (type === "refresh") {
      getListTable();
   }
...
const handleDeletePersonnelEducation = async () => {
   isLoading.value = true;
   const res = await deletePersonnelEducation(props?.personnelId, userEducationId?.value);
   if (res?.code === 20000) {
