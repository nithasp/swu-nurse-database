<template>
   ...
   >
      ...
      <div class="bg-white rounded-lg p-4 mb-5">
         <BarChart :idChart="`education-${index}`" :labels="['']" :datasets="data.datasets" labelY="จำนวนบุคลากร" labelX="ตำแหน่งวิชาการ" tooltipShow="personal-3" />
      </div>
      <div class="bg-white rounded-lg p-4">
         <div class="flex justify-end mb-4">
            <ButtonExport id="button-export" @click="exportFile('/dashboard/personnel/academicPosition/export', `ข้อมูลตำแหน่งวิชาการ-${dateFormExport()}`)" />
         </div>
         <UTable
            class="w-full"
            :ui="{
...
const { fetchDashboardPersonnelPosition } = useDashboardPersonalAPI;
...
const getDataChart = async () => {
   const res = await fetchDashboardPersonnelPosition();
   if (res.code == 20000) {
